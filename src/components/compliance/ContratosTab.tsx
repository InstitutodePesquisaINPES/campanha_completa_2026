...
import { AlertTriangle, Plus, Trash2, Pencil, FileText, ShieldCheck } from "lucide-react";
import { format } from "date-fns";
import { ContratoAprovacaoPanel } from "./ContratoAprovacaoPanel";
import { MinhasAprovacoesPendentes } from "./MinhasAprovacoesPendentes";
...
export function ContratosTab() {
  ...
  const abrirAprovacao = (id: string) => {
    ...
    if (c) { setContratoSel(c); setAprovacaoOpen(true); }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="grid lg:grid-cols-3 gap-4">
                  ...
                  {isLoading ? (
                    ...
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="flex gap-1">
                        <Button size="icon" variant="ghost" className="h-7 w-7" title="Workflow" onClick={() => abrirAprovacao(c.id)}><ShieldCheck className="h-3 w-3" /></Button>
        ...
        <div className="space-y-4">
          <MinhasAprovacoesPendentes onSelect={abrirAprovacao} />
        </div>
      </div>
      ...
      <Dialog open={aprovacaoOpen} onOpenChange={setAprovacaoOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Workflow: {contratoSel?.objeto}</DialogTitle>
          </DialogHeader>
          {contratoSel && <ContratoAprovacaoPanel contratoId={contratoSel.id} />}
        </DialogContent>
      </Dialog>
    </div>
  );
